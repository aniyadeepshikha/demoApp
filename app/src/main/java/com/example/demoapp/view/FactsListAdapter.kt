...
class FactsListAdapter(var context: Context?, var factsList: ArrayList<Details>?) :
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): FactsViewHolder {
        val view: View = LayoutInflater.from(context).inflate(R.layout.fact_list_item,parent, false)
    ...
    override fun onBindViewHolder(holder: FactsViewHolder, position: Int) {
        holder.tvTitle.setText(factsList!!.get(position).title?.toString());
        holder.tvDescription.setText(factsList!!.get(position).description?.toString());
        Glide.with(context!!).load(factsList!!.get(position).imageHref).fitCenter()
...
//            .error(R.drawable.imagenotfound)
    ...
    class FactsViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        init {
            ...
            ivImage = itemView.findViewById(R.id.ivNews)
        }
    }
}